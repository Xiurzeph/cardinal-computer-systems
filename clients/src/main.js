...
// Check for Demo Mode
const IS_DEMO_MODE = firebaseConfig.apiKey.includes("REPLACE");
        ...
        <div className="min-h-screen flex items-center justify-center px-4">
            <div className="max-w-md w-full bg-white shadow-xl rounded-lg p-8 border-t-8 border-cardinal-red">
                ...
                <div className="mt-6 text-center border-t pt-4">
                    <button onClick={() => switchMode(mode === 'client' ? 'admin' : 'client')} className="text-xs text-gray-400 hover:text-cardinal-red underline">
...
const Dashboard = ({ user, onLogout }) => {
    ...
    useEffect(() => {
        ...
        const docRef = db.collection('projects').doc(targetId);
        const unsubscribe = docRef.onSnapshot(doc => {
            if (doc.exists) {
                ...
                if (isAdmin) {
                    docRef.set({ projectPlan: initialProjectData, taskStates: {}, balanceText: "", createdAt: new Date() });
    ...
    const saveData = async (newPlan, newStates, newBalance) => {
        if (!isAdmin && !IS_DEMO_MODE) return;
    ...
    return (
        <div className="min-h-screen bg-zinc-50 font-sans text-cardinal-black pb-20">
            <header className="bg-white shadow-sm border-b sticky top-0 z-10 p-4">
                <div className="max-w-5xl mx-auto flex justify-between items-center flex-wrap gap-4">
                    <div className="flex items-center gap-3">
                        <img src="https://cardinalcomputersystems.com/cardinal-logo.png" className="w-10 h-10 object-contain" />
                        <div>
                    ...
                    <div className="flex items-center gap-4 ml-auto">
                        ...
                        {!isAdmin && (
                            <a href="https://www.paypal.com/ncp/payment/EPJHVX6XAGW8C" target="_blank" className="bg-cardinal-red text-white px-4 py-2 rounded text-xs font-bold hover:bg-zinc-800 transition-all shadow-sm">
                                Pay Invoice
                            </a>
                        )}
            ...
            <main className="max-w-5xl mx-auto px-4 py-8">
                            ...
                            <div className="p-5 grid md:grid-cols-2 gap-8">
                                {sprint.weeks.map((week, wI) => (
                                    <div key={week.id} className="bg-zinc-50 rounded-xl p-5 border border-zinc-100 flex flex-col">
                                        <h4 className="font-bold border-b border-zinc-200 pb-3 mb-5 text-gray-800 flex justify-between items-center">
                                            {week.title}
                ...
                <div className="mt-8 flex flex-wrap gap-8 justify-center text-[10px] uppercase font-bold tracking-tighter text-gray-400">
                    ...
                    <div className="flex items-center gap-2"><div className="w-3 h-3 bg-zinc-900 rounded"></div> In Progress</div>
...
const App = () => {
    ...
    if (loading) return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-zinc-100">
            <img src="https://cardinalcomputersystems.com/cardinal-logo.png" className="w-16 h-16 animate-pulse opacity-50 mb-4" />
